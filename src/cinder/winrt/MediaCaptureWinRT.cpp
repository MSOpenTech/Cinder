...
namespace WinRTMediaCapture
{
    ...
    void MediaCaptureWinRT::GetVideoCamerasAsync(GetMediaDevicesDelegate^ func)
    {
        ...
        create_task(DeviceInformation::FindAllAsync(DeviceClass::VideoCapture))
            .then([this, delegate](task<DeviceInformationCollection^> findTask)
        {
            Platform::Array<Platform::String^>^ devices = ref new Platform::Array<Platform::String^>(0);
            try
            {
                m_devInfoCollection = findTask.get();
                devices = ref new Platform::Array<Platform::String^>(m_devInfoCollection->Size);
                for (size_t i = 0; i < m_devInfoCollection->Size; i++)
                {
